...
class blockmodel:
    ...
    def modifytexture(self,property:dict,texture:str,layout:str):
        ...
        if thistype == "connecting":
            ...
            self.connections = addconnection(is_same_state="log" in self.name)
    ...
    def repickparent(self,blockmodels:dict):
        ...
        if self.isfullblock:
            ...
            raise NotImplementedError(f"{self.name}的方块不是完整的，实现会有问题")
            #获取带有元素的爹并直接修改元素内容
...
def addconnection(is_same_state:bool=False,is_face_visible:bool=False,match_block:dict=None) -> dict:
    """ 为纹理路径添加连接方法，效果为覆盖。
        不能完全归纳所需的情况。 """
...
class blockmodel_overlay:
    ...
    def __init__(self,property:dict,texture:str,layout:str,elements:list[dict] = None):
        ...
        for matchblock in matchblocks:
            temp = addconnection(is_face_visible=True,match_block=matchblock)
